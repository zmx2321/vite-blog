...
## 前言
- 之前介绍了通用的 UI 线程架构，每个 UI 线程都拥有一个消息队列，所有的待执行的事件都会被添加进消息队列中，UI 线程会按照一定规则，循环地取出消息队列中的事件，并执行事件。而 JavaScript 最初也是运行在 UI 线程中的。换句话说，JavaScript 语言就是基于这套通用的 UI 线程架构而设计的。
...
![](https://zmx2321.github.io/vite-blog/images/note/front/v8-note/19/19-1.png)
...
- 我们就先来打通微任务的底层技术，搞懂消息队列、主线程、调用栈的关联，然后抽丝剥茧地剖析微任务的实现机制。
...
![](https://zmx2321.github.io/vite-blog/images/note/front/v8-note/19/19-2.png)
- 然后 V8 便开始在主线程上执行 foo 函数，首先它会创建 foo 函数的执行上下文，并将其压入栈中，那么此时调用栈、主线程的关系如下图所示：
![](https://zmx2321.github.io/vite-blog/images/note/front/v8-note/19/19-3.png)
- 然后，foo 函数又调用了 bar 函数，那么当 V8 执行 bar 函数时，同样要创建 bar 函数的执行上下文，并将其压入栈中，最终效果如下图所示：
![](https://zmx2321.github.io/vite-blog/images/note/front/v8-note/19/19-4.png)
- 等 bar 函数执行结束，V8 就会从栈中弹出 bar 函数的执行上下文，此时的效果如下所示：
![](https://zmx2321.github.io/vite-blog/images/note/front/v8-note/19/19-5.png)
- 最后，foo 函数执行结束，V8 会将 foo 函数的执行上下文从栈中弹出，效果如下所示：
![](https://zmx2321.github.io/vite-blog/images/note/front/v8-note/19/19-6.png)
...
![](https://zmx2321.github.io/vite-blog/images/note/front/v8-note/19/19-7.png)
...
![](https://zmx2321.github.io/vite-blog/images/note/front/v8-note/19/19-8.png)
- 接下来 V8 就要执行 foo 函数了，同样执行 foo 函数时，会创建 foo 函数的执行上下文，并将其压入栈中，最终效果如下图所示：
![](https://zmx2321.github.io/vite-blog/images/note/front/v8-note/19/19-9.png)
- 当 V8 执行执行 foo 函数中的 setTimeout 时，setTimeout 会将 foo 函数封装成一个新的宏任务，并将其添加到消息队列中，在 V8 执行 setTimeout 函数时的状态图如下所示：
![](https://zmx2321.github.io/vite-blog/images/note/front/v8-note/19/19-10.png)
- 等 foo 函数执行结束，V8 就会结束当前的宏任务，调用栈也会被清空，调用栈被清空后状态如下图所示：
![](https://zmx2321.github.io/vite-blog/images/note/front/v8-note/19/19-11.png)
...
![](https://zmx2321.github.io/vite-blog/images/note/front/v8-note/19/19-12.png)
...
![](https://zmx2321.github.io/vite-blog/images/note/front/v8-note/19/19-13.png)
...
![](https://zmx2321.github.io/vite-blog/images/note/front/v8-note/19/19-14.png)
...
- 此时的消息队列、主线程和调用栈的状态图如下所示：
![](https://zmx2321.github.io/vite-blog/images/note/front/v8-note/19/19-15.png)
...
- 此时的消息队列、主线程和调用栈的状态图如下所示：
![](https://zmx2321.github.io/vite-blog/images/note/front/v8-note/19/19-16.png)
...
- 此时的消息队列、主线程和调用栈的状态图如下所示：
![](https://zmx2321.github.io/vite-blog/images/note/front/v8-note/19/19-17.png)
...
- 此时的消息队列、主线程和调用栈的状态图如下所示：
![](https://zmx2321.github.io/vite-blog/images/note/front/v8-note/19/19-18.png)
...
![](https://zmx2321.github.io/vite-blog/images/note/front/v8-note/19/19-19.png)
...
- 浏览器中的 MutationObserver 接口提供了监视对 DOM 树所做更改的能力，它在内部也使用了微任务的技术，那么今天留给你的作业是，查找 MutationObserver 相关资料，分析它是如何工作的，其中微任务的作用是什么？
    - MutationObserver和IntersectionObserver两个性质应该差不多。这里简称ob。ob是一个微任务，通过浏览器的requestIdleCallback，在浏览器每一帧的空闲时间执行ob监听的回调，该监听是不影响主线程的，但是回调会阻塞主线程。当然有一个限制，如果100ms内主线程一直处于未空闲状态，那会强制触发ob。
