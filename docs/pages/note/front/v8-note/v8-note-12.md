...
## js代码不同的执行场景
- 在使用 JavaScript 的过程中，经常会遇到栈溢出的错误，比如执行下面这样一段代码：
```js
function foo() {
 foo() // 是否存在堆栈溢出错误?
}
...
- 接下来我们来分析不同的函数调用方式是如何影响到运行时内存布局的。
- 下图是本文的主要内容在编译流水线中的位置，因为解释执行和直接执行二进制代码都使用了堆和栈，虽然它们在执行细节上存在着一定的差异，但是整体的执行架构是类似的。
![](https://zmx2321.github.io/vite-blog/images/note/front/v8-note/12/12-1.png)
...
![嵌套调用时函数的生命周期](https://zmx2321.github.io/vite-blog/images/note/front/v8-note/12/12-2.png)
...
![嵌套调用时函数的生命周期](https://zmx2321.github.io/vite-blog/images/note/front/v8-note/12/12-3.png)
...
![数据结构层面的栈](https://zmx2321.github.io/vite-blog/images/note/front/v8-note/12/12-4.png)

## 栈如何管理函数调用
- 首先我们来分析最简单的场景：当执行一个函数的时候，栈怎么变化？
- 当一个函数被执行时，函数的参数、函数内部定义变量都会依次压入到栈中，我们结合实际的代码来分析下这个过程，你可以参考下图：
![函数内部变量压栈状态](https://zmx2321.github.io/vite-blog/images/note/front/v8-note/12/12-5.png)
...
![](https://zmx2321.github.io/vite-blog/images/note/front/v8-note/12/12-6.png)
- 接下来，就要调用 add 函数了，理想状态下，执行 add 函数的过程是下面这样的：
![](https://zmx2321.github.io/vite-blog/images/note/front/v8-note/12/12-7.png)
...
![add函数即将执行结束的状态](https://zmx2321.github.io/vite-blog/images/note/front/v8-note/12/12-8.png)
![恢复mian函数执行现场](https://zmx2321.github.io/vite-blog/images/note/front/v8-note/12/12-9.png)
...
![ebp寄存器保存了栈帧指针](https://zmx2321.github.io/vite-blog/images/note/front/v8-note/12/12-10.png)
...
![](https://zmx2321.github.io/vite-blog/images/note/front/v8-note/12/12-11.png)
...
![](https://zmx2321.github.io/vite-blog/images/note/front/v8-note/12/12-12.png)
...
- 微任务，包括Promise、MutationObserver等，是在浏览器的主线程中执行的，一直拉取微任务，这样的循环会不断创建栈，再销毁栈，所以不会爆栈，但是因为在主线程中执行，所以页面会卡死。
    - 主线程在当前任务快要执行结束之前，检查微任务队列中是否存在微任务，如果有，那么那么那么当前任务会依次取出微任务队列中的微任务，并一一执行！
    - 微任务队列是属于当前宏任务的，所以一个宏任务中产生的微任务，只会放到它自己的微任务队列中！
    - 微任务并没有进消息队列，它只是当前任务的一个未完成部分.
    - 因为promise触发的微任务会一直在当前的任务中执行，这回导致其他的宏任务无法被执行！ 所以就造成了页面的卡死
