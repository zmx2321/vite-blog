# V8是如何优化垃圾回收器执行效率的
> 之前我们介绍了 V8 使用副垃圾回收器和主垃圾回收器来处理垃圾回收，现在我们看看 V8 是如何优化垃圾回收器的执行效率的。
...
![](https://zmx2321.github.io/vite-blog/images/note/front/v8-note/22/22-1.png)
...
![](https://zmx2321.github.io/vite-blog/images/note/front/v8-note/22/22-2.png)
...
- 所谓增量式垃圾回收，是指垃圾收集器将标记工作分解为更小的块，并且穿插在主线程不同的任务之间执行。采用增量垃圾回收时，垃圾回收器没有必要一次执行完整的垃圾回收过程，每次执行的只是整个垃圾回收过程中的一小部分工作，具体流程可以参看下图：
![](https://zmx2321.github.io/vite-blog/images/note/front/v8-note/22/22-3.png)
...
![](https://zmx2321.github.io/vite-blog/images/note/front/v8-note/22/22-4.png)
...
![](https://zmx2321.github.io/vite-blog/images/note/front/v8-note/22/22-5.png)
...
![](https://zmx2321.github.io/vite-blog/images/note/front/v8-note/22/22-6.png)
- 然后又执行了另外一个代码，这段代码如下所示：
```js
window.a.b = Object() //d
```
- 执行完之后，垃圾回收器又恢复执行了增量标记过程，由于 b 重新指向了 d 对象，所以 b 和 c 对象的连接就断开了。这时候代码的应用如下图所示：
![](https://zmx2321.github.io/vite-blog/images/note/front/v8-note/22/22-7.png)
...
![](https://zmx2321.github.io/vite-blog/images/note/front/v8-note/22/22-8.png)
...
## 思考
- 虽然 V8 为执行垃圾回收的效率做了大量的优化，但是在实际项目中我们依然要关心内存问题，那么在使用 JavaScript 时，如何避免内存泄漏？
    ...
    - 避免没错泄露很重要一点就是避免对象的循环引用，这个很容易造成这个对象永远不会被标记而持续占用内存，同时大对象或大数组最好在使用完之后赋值为undefined释放内存～还有一点就是使用框架的时候对事件的监听要在组件销毁的同时移除监听，否则这个监听的回调函数所占用内存也是不会释放的
    - 别把对象关联到全局变量上，避免循环引用
