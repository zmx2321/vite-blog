# 几种常见内存问题的解决策略
> 本章我们来结合 Node 中的读文件操作，分析下消息循环系统是怎么影响到异步编程的，然后我们再来结合 JavaScript 中的几种常见的内存问题，来分析下内存问题出现的原因和解决方法。

## Node 中的 readFile API 工作机制
- 我们通过这两段代码来分析下同步和异步读文件 API 的区别。
```js
// 代码一
var fs = require('fs')
...
![](https://zmx2321.github.io/vite-blog/images/note/front/v8-note/23/23-1.png)
...
![](https://zmx2321.github.io/vite-blog/images/note/front/v8-note/23/23-2.png)
...
![](https://zmx2321.github.io/vite-blog/images/note/front/v8-note/23/23-3.png)
- 等到 libuv 从消息队列中读取该事件后，主线程就可以着手来处理该事件了。在主线程处理该事件的过程中，主线程调用事件中的回调函数，并将 data 结果数据作为参数，如下图所示：
![](https://zmx2321.github.io/vite-blog/images/note/front/v8-note/23/23-4.png)
...
- 为了解决这个问题，我们可以在 JavaScript 文件头部加上use strict，使用严格模式避免意外的全局变量，此时上例中的 this 指向 undefined。
...
![](https://zmx2321.github.io/vite-blog/images/note/front/v8-note/23/23-5.png)
...
![](https://zmx2321.github.io/vite-blog/images/note/front/v8-note/23/23-6.png)
...
![](https://zmx2321.github.io/vite-blog/images/note/front/v8-note/23/23-7.png)
