...
## 前言
- 学习字节码如何被执行，可以让我们理解解释器的工作机制，同时还能帮助我们搞懂 JavaScript 运行时的内存结构，特别是闭包的结构和非闭包数据的区别。
- 字节码的解释执行在编译流水线中的位置你可以参看下图：
![](https://zmx2321.github.io/vite-blog/images/note/front/v8-note/15/15-1.png)
...
```
- 同样，我们将其图形化：
![](https://zmx2321.github.io/vite-blog/images/note/front/v8-note/15/15-2.png)
- 从图中可以看出，函数的字面量被解析为 AST 树的形态，这个函数主要拆分成四部分。
    1. 第一部分为参数的声明 (PARAMS)，参数声明中包括了所有的参数，在这里主要是参数 x 和参数 y，你可以在函数体中使用 arguments 来使用对应的参数。
    2. 第二部分是变量声明节点 (DECLS)，参数部分你可以使用 arguments 来调用，同样，你也可以将这些参数作为变量来直接使用，这体现在 DECLS 节点下面也出现了变量 x 和变量 y，除了可以直接使用 x 和 y 之外，我们还有一个 z 变量也在 DECLS 节点下。你可以注意一下，在上面生成的 AST 数据中，参数声明节点中的 x 和变量声明节点中的 x 的地址是相同的，都是 0x7fa7bf8048e8，同样 y 也是相同的，都是 0x7fa7bf804990，这说明它们指向的是同一块数据。
    3. 第三部分是 x+y 的表达式节点，我们可以看到，节点 add 下面使用了 var proxy x 和 var proxy x 的语法，它们指向了实际 x 和 y 的值。
    4. 第四部分是 RETURN 节点，它指向了 z 的值，在这里是 local[0]。
...
![](https://zmx2321.github.io/vite-blog/images/note/front/v8-note/15/15-3.png)
...
![V8中定义的部分字节码指令集](https://zmx2321.github.io/vite-blog/images/note/front/v8-note/15/15-4.png)
- 也可以去[V8 的源码](https://github.com/v8/v8/blob/master/src/interpreter/bytecodes.h)中查看这些字节码，V8 字节码的指令非常多，如果要掌握所有指令的含义，需要花费一段时间的学习和实践，这节课我们不需要了解所有字节码的含义，但我们需要知道，怎样阅读字节码。
...
![](https://zmx2321.github.io/vite-blog/images/note/front/v8-note/15/15-5.png)
...
![](https://zmx2321.github.io/vite-blog/images/note/front/v8-note/15/15-6.png)
- 们再来看另外一个段字节码指令：
```js
Star r0
```
...
![](https://zmx2321.github.io/vite-blog/images/note/front/v8-note/15/15-7.png)
- 我们再来看一个执行加法的字节码：
```js
Add a0, [0]
```
- Add a0, [0]是从 a0 寄存器加载值并将其与累加器中的值相加，然后将结果再次放入累加器，最终操作如下图所示：
![](https://zmx2321.github.io/vite-blog/images/note/front/v8-note/15/15-8.png)
...
![](https://zmx2321.github.io/vite-blog/images/note/front/v8-note/15/15-9.png)
- 我们再来看一个字节码：
```js
Return
```
...
```
- 执行这段代码时，整体的状态如下图所示：
![](https://zmx2321.github.io/vite-blog/images/note/front/v8-note/15/15-10.png)
- 我们可以看到：
